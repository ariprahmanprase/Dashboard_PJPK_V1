          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(220px, 1fr))', gap: '1rem', alignItems: 'end' }}>
            ...
            {[podium[1], podium[0], podium[2]].map((row, idx) => {
              ...
              return (
                ...
                >
                  <div style={{ width: 52, height: 52, margin: '0 auto 0.75rem', borderRadius: '999px', background: m.bg, display: 'grid', placeItems: 'center' }}>
                    {rank === 1 ? <Trophy size={26} style={{ color: m.color }} />
                      : rank === 2 ? <Medal size={26} style={{ color: m.color }} />
                      : <Award size={26} style={{ color: m.color }} />}
                  </div>
                  <p style={{ fontSize: '0.688rem', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.06em', color: m.color }}>{m.label}</p>
                  <p className="font-bold" style={{ color: 'hsl(var(--ds-foreground))', marginTop: '0.25rem', fontSize: isFirst ? '1.05rem' : '0.95rem', lineHeight: 1.3 }}>{row.opd}</p>
                  <p style={{ fontSize: '1.75rem', fontWeight: 800, color: 'hsl(var(--ds-primary))', marginTop: '0.5rem', fontVariantNumeric: 'tabular-nums' }}>{row.pct_tercapai}%</p>
                  <p style={{ fontSize: '0.688rem', color: 'hsl(var(--ds-muted-foreground))' }}>{row.renaksi_tercapai} dari {row.renaksi_total} renaksi tercapai</p>
                </div>
              );
            })}
          </div>
