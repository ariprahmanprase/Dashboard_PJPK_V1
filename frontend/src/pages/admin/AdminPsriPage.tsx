...
import { Loader2, RefreshCw, Stethoscope, Trash2 } from 'lucide-react';
...
/**
 * P8 — PSRI Policy Diagnosis (wizard):
 * pilih indikator + tahun → Generate → hasil tersimpan (bisa regenerate / hapus).
 * Menganalisis Pressure–State–Response–Impact dan koherensi intervensi.
 */
export default function AdminPsriPage({ user, onLogout, onNavigate }: Props) {
  ...
  const muatHasilTersimpan = useCallback(async (k: string, t: string) => {
    ...
    setError('');
    try {
      setHasil(await fetchAiPsri(k, t));
    } catch (e) {
  ...
  const handleGenerate = async () => {
    ...
    setError('');
    try {
      setHasil(await generateAiPsri(kode, tahun));
    } catch (e) {
      setError(e instanceof Error ? e.message : 'Gagal membuat diagnosis.');
    } finally {
      setProses(false);
    }
  };
  ...
  const handleHapus = async () => {
    ...
    if (!window.confirm('Hapus PSRI diagnosis tersimpan untuk indikator & tahun ini?')) return;
    setProses(true);
    setError('');
    try {
      await deleteAiPsri(kode, tahun);
      setHasil(null);
    } catch (e) {
      setError(e instanceof Error ? e.message : 'Gagal menghapus diagnosis.');
    } finally {
      setProses(false);
    }
  };
  ...
  return (
    ...
    >
      ...
      >
        <div className="flex flex-col gap-5">
          ...
          <div>
            ...
            <p className="mt-2.5 text-xs" style={{ color: 'var(--color-text-secondary)' }}>
              Menganalisis kerangka kausal kebijakan: Pressure → State → Response → Impact, lalu menguji koherensi intervensi.
            </p>
          </div>
          ...
          <div className="flex flex-wrap gap-4 items-end">
            ...
            >
              {proses ? <Loader2 size={16} className="animate-spin" /> : (hasil ? <RefreshCw size={16} /> : <Stethoscope size={16} />)}
              {proses ? 'Memproses…' : hasil ? 'Regenerate' : 'Generate Diagnosis'}
            </button>
          </div>
        </div>
      </div>
      ...
      >
        {!kode ? (
          <Placeholder>Pilih indikator terlebih dahulu untuk melihat atau membuat PSRI diagnosis.</Placeholder>
        ) : muatHasil ? (
          <Placeholder><Loader2 size={16} className="inline animate-spin mr-2" />Memuat diagnosis tersimpan…</Placeholder>
        ) : hasil ? (
          <>
          ...
          <Placeholder>
            Belum ada PSRI diagnosis untuk indikator & tahun ini. Klik <strong>Generate Diagnosis</strong> untuk membuatnya.
          </Placeholder>
        )}
      </div>
      </div>
