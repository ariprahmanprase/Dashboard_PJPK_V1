...
import {
  deleteAiEfektivitas,
  fetchAiEfektivitas,
  fetchAiEfektivitasOptions,
  generateAiEfektivitas,
  type AdminUser,
...
/**
 * P4 — Activity-Outcome Effectiveness (wizard):
 * pilih indikator + tahun → Generate → hasil tersimpan (bisa regenerate / hapus).
 * Menguji apakah kegiatan benar-benar berkontribusi terhadap outcome indikator.
 */
export default function AdminEfektivitasPage({ user, onLogout, onNavigate }: Props) {
  ...
  const muatHasilTersimpan = useCallback(async (k: string, t: string) => {
    ...
    setError('');
    try {
      setHasil(await fetchAiEfektivitas(k, t));
    } catch (e) {
  ...
  const handleGenerate = async () => {
    ...
    setError('');
    try {
      setHasil(await generateAiEfektivitas(kode, tahun));
    } catch (e) {
      setError(e instanceof Error ? e.message : 'Gagal membuat evaluasi.');
    } finally {
      setProses(false);
    }
  };
  ...
  const handleHapus = async () => {
    ...
    if (!window.confirm('Hapus evaluasi efektivitas tersimpan untuk indikator & tahun ini?')) return;
    setProses(true);
    setError('');
    try {
      await deleteAiEfektivitas(kode, tahun);
      setHasil(null);
    } catch (e) {
      setError(e instanceof Error ? e.message : 'Gagal menghapus evaluasi.');
    } finally {
      setProses(false);
    }
  };
  ...
  return (
    ...
    >
      ...
      >
        <div className="flex flex-col gap-5">
          ...
          <div>
            ...
            <p className="mt-2.5 text-xs" style={{ color: 'var(--color-text-secondary)' }}>
              Menguji apakah kegiatan yang dijalankan benar-benar berkontribusi terhadap outcome indikator.
            </p>
          </div>
          ...
          <div className="flex flex-wrap gap-4 items-end">
            ...
            >
              {proses ? <Loader2 size={16} className="animate-spin" /> : (hasil ? <RefreshCw size={16} /> : <Gauge size={16} />)}
              {proses ? 'Memproses…' : hasil ? 'Regenerate' : 'Generate Evaluasi'}
            </button>
          </div>
        </div>
      </div>
      ...
      >
        {!kode ? (
          <Placeholder>Pilih indikator terlebih dahulu untuk melihat atau membuat evaluasi efektivitas.</Placeholder>
        ) : muatHasil ? (
          <Placeholder><Loader2 size={16} className="inline animate-spin mr-2" />Memuat evaluasi tersimpan…</Placeholder>
        ) : hasil ? (
          <>
          ...
          <Placeholder>
            Belum ada evaluasi efektivitas untuk indikator & tahun ini. Klik <strong>Generate Evaluasi</strong> untuk membuatnya.
          </Placeholder>
        )}
      </div>
      </div>
