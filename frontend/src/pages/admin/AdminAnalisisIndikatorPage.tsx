...
/**
 * P1 — Indicator Performance Analysis (wizard):
 * pilih indikator + tahun → Generate → hasil tersimpan (bisa regenerate / hapus).
 */
export default function AdminAnalisisIndikatorPage({ user, onLogout, onNavigate }: Props) {
  ...
  const muatHasilTersimpan = useCallback(async (k: string, t: string) => {
    ...
    setError('');
    try {
      setHasil(await fetchAiIndikator(k, t));
    } catch (e) {
  ...
  const handleGenerate = async () => {
    ...
    setError('');
    try {
      setHasil(await generateAiIndikator(kode, tahun));
    } catch (e) {
  ...
  const handleHapus = async () => {
    ...
    if (!window.confirm('Hapus analisis tersimpan untuk indikator & tahun ini?')) return;
    setProses(true);
    setError('');
    try {
      await deleteAiIndikator(kode, tahun);
      setHasil(null);
    } catch (e) {
  ...
  return (
    ...
    >
      ...
      >
        {!kode ? (
          <Placeholder>Pilih indikator terlebih dahulu untuk melihat atau membuat analisis.</Placeholder>
        ) : muatHasil ? (
          ...
          <Placeholder>
            Belum ada analisis untuk indikator & tahun ini. Klik <strong>Generate Analisis</strong> untuk membuatnya.
          </Placeholder>
        )}
      </div>
      </div>
