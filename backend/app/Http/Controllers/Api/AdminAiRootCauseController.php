...
use App\Services\AiRootCauseAnalysisService;
use Illuminate\Http\Request;

/**
 * P3 — Root Cause Analysis (admin area).
 * Hak akses: admin OPD hanya indikator yang tertaut ke dinasnya
 * (super_admin mewakili Bappeda — akses lintas OPD).
 */
class AdminAiRootCauseController extends Controller
{
    /**
     * Dropdown indikator untuk wizard P3.
    ...
    /**
     * Ambil analisis P3 tersimpan untuk indikator + tahun.
     */
    public function show(Request $request)
    {
        ...
        $analisis = AiAnalysis::where('prompt_kode', 'P3')
    ...
    /**
     * Generate (atau regenerate) analisis P3 lalu simpan ke DB.
     */
    public function generate(Request $request, AiRootCauseAnalysisService $service)
    {
        $request->validate([
            'kode'  => ['required', 'string', 'exists:indikators,kode'],
            'tahun' => ['required', 'digits:4'],
        ...
        $analisis = AiAnalysis::updateOrCreate(
            ['prompt_kode' => 'P3', 'indikator_id' => $indikator->id, 'tahun' => $tahun],
            [
        ...
        return response()->json([
            'message' => 'Root cause analysis berhasil dibuat.',
            'data'    => [
                'hasil'      => $analisis->hasil,
                'model'      => $analisis->model,
                'updated_at' => $analisis->updated_at?->format('d M Y H:i'),
    ...
    public function destroy(Request $request)
    {
        ...
        AiAnalysis::where('prompt_kode', 'P3')
            ->where('indikator_id', $indikator->id)
            ->where('tahun', $request->input('tahun'))
            ->delete();
