...
use App\Services\AiPlanningBudgetService;
use Illuminate\Http\Request;

/**
 * P11 — Planning & Budget Alignment (admin area).
 *
 * STATUS: BELUM DITAMPILKAN DI UI (lihat catatan di AiPlanningBudgetService).
 * Backend lengkap (endpoint bisa diakses lewat API), tetapi belum ada
 * halaman/menu frontend. Diaktifkan setelah data dokumen perencanaan &
 * anggaran tersedia di DB.
 *
 * Hak akses: admin OPD hanya indikator yang tertaut ke dinasnya
 * (super_admin mewakili Bappeda — akses lintas OPD).
 */
class AdminAiPlanningBudgetController extends Controller
{
    /**
     * Dropdown indikator + tahun.
     */
    ...
    /**
     * Ambil analisis P11 tersimpan untuk indikator + tahun.
     */
    public function show(Request $request)
    {
        ...
        $analisis = AiAnalysis::where('prompt_kode', 'P11')
    ...
    /**
     * Generate (atau regenerate) analisis P11 lalu simpan ke DB.
     */
    public function generate(Request $request, AiPlanningBudgetService $service)
    {
        [$indikator, $tahun, $resp] = $this->resolve($request);
        if ($resp) {
            return $resp;
        ...
        $analisis = AiAnalysis::updateOrCreate(
            ['prompt_kode' => 'P11', 'indikator_id' => $indikator->id, 'tahun' => $tahun],
            [
        ...
        return response()->json([
            'message' => 'Analisis planning & budget alignment berhasil dibuat.',
            'data'    => [
                'hasil'      => $analisis->hasil,
                'model'      => $analisis->model,
                'updated_at' => $analisis->updated_at?->format('d M Y H:i'),
    ...
    public function destroy(Request $request)
    {
        ...
        AiAnalysis::where('prompt_kode', 'P11')
            ->where('indikator_id', $indikator->id)
            ->where('tahun', $tahun)
            ->delete();
